    ...
    <div class="view view-login">
      ...
      <div class="login-body">
        <div class="login-card">
          ...
          <button type="button" id="google-signin-btn" class="btn btn-google" aria-label="Se connecter avec Google">
            <svg class="google-icon" viewBox="0 0 24 24" width="22" height="22" aria-hidden="true" focusable="false">
        ...
        <div class="login-footer-links">
          <button type="button" class="link-btn" id="privacy-link-login">Politique de confidentialité</button>
        </div>
      </div>
      <footer class="app-footer">
        <p>&copy; <span class="dyn-year"></span> Bryan Drouet &mdash; Tous droits réservés</p>
      </footer>
    </div>
  `;
  document.getElementById("google-signin-btn")?.addEventListener("click", handleGoogleSignIn);
  document.getElementById("privacy-link-login")?.addEventListener("click", openPrivacyModal);
  updateYears();
  initIcons();
}
...
async function renderDashboard(filterCategory = null) {
  ...
  const avatarHtml = currentUser.photoURL
    ? `<img src="${currentUser.photoURL}" alt="Photo de profil de ${currentUser.displayName || "utilisateur"}" class="user-avatar">`
    : "";
  app.innerHTML = `
    <div class="view view-dashboard">
      <header class="app-header">
        ...
        <div class="header-right">
          ...
          <button type="button" id="signout-btn" class="btn btn-outline btn-sm" aria-label="Se déconnecter">
      ...
      <main class="dashboard-body">
        <div class="dashboard-toolbar">
          <div class="category-filters" id="category-filters" role="group" aria-label="Filtrer par catégorie">
      ...
      <footer class="app-footer">
        <p>&copy; <span class="dyn-year"></span> Bryan Drouet &mdash; Tous droits réservés</p>
        <button type="button" class="link-btn" id="privacy-link-dash">Politique de confidentialité</button>
      </footer>
    </div>
  `;
  document.getElementById("signout-btn")?.addEventListener("click", handleSignOut);
  document.getElementById("create-bingo-btn")?.addEventListener("click", () => renderCreateView());
  document.getElementById("privacy-link-dash")?.addEventListener("click", openPrivacyModal);
  updateYears();
  initIcons();
...
function renderCategoryFilters(categories, active) {
  ...
  if (!el) return;
  const items = [
    `<button type="button" class="chip ${!active ? "chip-active" : ""}" data-cat="">Tous</button>`,
    ...categories.map(c => `<button type="button" class="chip ${active === c ? "chip-active" : ""}" data-cat="${c}">${c}</button>`)
  ];
  el.innerHTML = items.join("");
  el.querySelectorAll(".chip").forEach(btn => {
    btn.addEventListener("click", () => renderDashboard(btn.dataset.cat || null));
  });
}
  ...
  el.innerHTML = bingos.map(b => `
    <article class="bingo-card" data-id="${b.id}" tabindex="0" role="button" aria-label="Ouvrir le bingo ${b.title}">
      ...
      <div class="bingo-card-actions">
        ...
        <button type="button" class="btn btn-neutral btn-sm js-edit" data-id="${b.id}" aria-label="Modifier ${b.title}">
          <i data-lucide="pencil" aria-hidden="true"></i>
        </button>
        <button type="button" class="btn btn-danger btn-sm js-delete" data-id="${b.id}" aria-label="Supprimer ${b.title}">
...
async function renderCreateView(editId = null) {
  ...
  app.innerHTML = `
    <div class="view view-create">
      <header class="app-header">
        <div class="header-left">
          <button type="button" id="back-btn" class="btn-back" aria-label="Retour">
...
function renderPlayBoard(bingo) {
  ...
  app.innerHTML = `
    <div class="view view-play">
      <header class="app-header">
        <div class="header-left">
          <button type="button" id="back-play-btn" class="btn-back" aria-label="Retour au tableau de bord">
            <i data-lucide="arrow-left" aria-hidden="true"></i>
          </button>
          <div class="header-title-group">
        ...
        <div class="header-right">
          <button type="button" id="reset-btn" class="btn btn-outline btn-sm btn-icon-only" aria-label="Réinitialiser les cases cochées">
      ...
      <footer class="app-footer">
        <p>Bingo conçu et rempli par Bryan_Drouet &mdash; &copy; <span class="dyn-year"></span></p>
      </footer>
    </div>
  `;
